...
#include "include/sharp_ylmgen_c.h"
#include "include/sharp_internal.h"
#include "include/sharp_utils.h"
#include "include/sharp_almhelpers.h"
#include "include/sharp_geomhelpers.h"
...
void hmap2alm(double* map, double complex* alm, size_t nside, size_t lmax, size_t mmax, int32_t add){
  void* aptr=(void*)alm; 
  void* mptr=(void*)map;

  int flags=SHARP_DP | (add ? SHARP_ADD : 0);
...
void hmap2alm_spin (double *map1, double *map2, double complex* alm1, double complex* alm2, int spin, size_t nside, size_t lmax, size_t mmax, int32_t add)
{
  void *aptr[2], *mptr[2];
  aptr[0]=(void*)(alm1); aptr[1]=(void*)(alm2);
  mptr[0]=(void*)(map1); mptr[1]=(void*)(map2);
  int flags=SHARP_DP | (add ? SHARP_ADD : 0);
  sharp_geom_info *ginfo;
  sharp_alm_info *ainfo;
  sharp_make_healpix_geom_info (nside, 1, &ginfo);
...
void alm2hmap(double complex* alm, double* map, size_t nside, size_t lmax, size_t mmax, int32_t add){
  void* aptr=(void*)alm;
  void* mptr=(void*)map;
  int flags=SHARP_DP | (add ? SHARP_ADD : 0);
  sharp_geom_info *ginfo;
...
void alm2hmap_spin (double complex* alm1, double complex* alm2,
...
/////float//////
void hmap2alm_f(float* map, float complex* alm, size_t nside, size_t lmax, size_t mmax, int32_t add){
  void* aptr=(void*)alm; 
  void* mptr=(void*)map;

  int flags=0 | (add ? SHARP_ADD : 0);
  sharp_geom_info *ginfo;
  sharp_alm_info *ainfo;
  sharp_make_healpix_geom_info (nside, 1, &ginfo);
  sharp_make_triangular_alm_info (lmax, mmax, 1, &ainfo);
...
void hmap2alm_spin_f (float *map1, float *map2, float complex* alm1, float complex* alm2, int spin, size_t nside, size_t lmax, size_t mmax, int32_t add)
{
  void *aptr[2], *mptr[2];
  aptr[0]=(void*)(alm1); aptr[1]=(void*)(alm2);
  mptr[0]=(void*)(map1); mptr[1]=(void*)(map2);
...
void alm2hmap_f(float complex* alm, float* map, size_t nside, size_t lmax, size_t mmax, int32_t add){
  void* aptr=(void*)alm;
  void* mptr=(void*)map;
  int flags=0 | (add ? SHARP_ADD : 0);
  sharp_geom_info *ginfo;
...
void alm2hmap_spin_f (float complex* alm1, float complex* alm2,
